...
describe('passwd', () => {
  ...
  it('must have a minimum of 3 characters', () => {
    ...
    expect(passwd.checkRules('13')).toEqual(['Minimum length is 3'])
    expect(passwd.checkRules('133')).toEqual([])
  })

  it('must have a maximum of 10 characters', () => {
    process.env.MD_PASSWORD_RULES = 'maxlength:10'
    expect(passwd.checkRules('12345678901')).toEqual(['Maximum length is 10'])
  ...
  it('must have a minimum of 2 numeric characters', () => {
    ...
    expect(passwd.checkRules('abc1def')).toEqual(['It must contain at least 2 digits'])
  ...
  it('must have a minimum of 2 uppercase characters', () => {
    ...
    expect(passwd.checkRules('Abc1def')).toEqual(['It must contain at least 2 uppercase characters'])
  ...
  it('must have a minimum of 2 special characters', () => {
    ...
    expect(passwd.checkRules('Abc@def')).toEqual(['It must contain at least 2 special characters - !@#$%'])
    expect(passwd.checkRules('Abc#$Bf2')).toEqual([])
  })

  it('must not have invalid characters', () => {
    process.env.MD_PASSWORD_RULES = 'minlength:1'
    expect(passwd.checkRules('@#^[/')).toEqual(['It must contain only valid characters: 0-9, A-Z, a-z, !@#$%'])
  })
