...
const minlength = (text, param) => {
  if(text.length < param) return `Minimum length is ${param}`
}

const maxlength = (text, param) => {
  if(text.length > param) return `Maximum length is ${param}`
}

const digits = (text, param) => {
  if(text.replace(/[^0-9]/g,"").length < param) return `It must contain at least ${param} digit${param > 1 ? 's' : ''}`
}

const caps = (text, param) => {
  if(text.replace(/[^A-Z]/g,"").length < param) return `It must contain at least ${param} uppercase character${param > 1 ? 's' : ''}`
}

const special = (text, param) => {
  if(text.replace(/[^!@#$%]/g,"").length < param) return `It must contain at least ${param} special character${param > 1 ? 's' : ''} - !@#$%`
}

const invalid = (text) => {
  if(text.replace(/[0-9a-zA-z!@#$%]/g,"").length > 0) return `It must contain only valid characters: 0-9, A-Z, a-z, !@#$%`
}
